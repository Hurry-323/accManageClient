	...
	<el-card>
		<el-form ref="editPwdFormRef"  :model="editPwdForm" :rules="editPwdFormRules" label-width="0px" class="editPwd_form">
			...
			<el-form-item prop="accountId">
				<el-input v-model="editPwdForm.accountId" prefix-icon="el-icon-user-solid" 
				 disabled></el-input>
			</el-form-item>
			<!-- 密码 -->
			<el-form-item prop="password">
				<el-input v-model="editPwdForm.password" prefix-icon="el-icon-lock" 
	...
	export default{
		data() {
			...
			var checkPass = (rule, value, callback) => {
				...
				} else if (this.checkPassword== this.editPwdForm.password) {
		...
		methods:{
			...
			editPwd(){
				this.$refs.editPwdFormRef.validate(async valid => {
					...
					const {data : res} = await this.$http.post("/personInfo/editPassword",this.editPwdForm);
					//打印后台返回的数据
					console.log(res);
					//判断是否成功
					if(res.code == '8888'){
			...
			init0(){
				//用户名
				this.editPwdForm.accountId = window.sessionStorage.getItem('accountId');	
			},
			//注销
			logout () {
			  // 清除token
