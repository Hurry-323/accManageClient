	...
	data() {
		...
		methods:{
			async getUserList (){
				this.userlist = [];
				//获取后端用户数据
				const { data: res } = await this.$http.get('/account/findAccountsByStatusId', {params: {statusId: "1"}})
				//打印后台的返回值
				console.log(res);
			...
			addUser(){
				...
				// {
				// 	if(!valid) valid
				// 	const {data: res} = await this.$http.get('/User/findAllUser',this.addForm)
				...
				// if (!valid) return
				// // 可以发起添加用户的网络请求
				// this.$store.commit("adduser",{userName:this.addForm.username,
				// passWord:this.addForm.password,email:this.addForm.email,mobile:this.addForm.mobile});
				// this.addDialogVisible = false;
				// this.getUserList()
				// })
			},
